					...
					local Connection1; Connection1 = RightArm.ChildAdded:Connect(function(obj)
						...
						if obj.Name == "RightGrip" and obj.Part1 and obj.Part1.Parent and obj.Part1.Parent:IsA("Tool") then
							...
							if DangerousTools[NewTool] then
								if Toggles["antibypassGuxx"] == false then return end
								obj.Part1:Destroy()
								obj.Part0 = nil
								obj.Part1 = nil
								task.spawn(function()
...
LocalPlayer.Chatted:Connect(function(msg)
	local PrefixLen = (((msg:sub(1, #Prefix) == Prefix) and #Prefix) or ((msg:sub(1, #ankyewemkmthu) == ankyewemkmthu) and #ankyewemkmthu)) or nil
...
do
	...
	local Namecall; Namecall = hookmetamethod(game, "__namecall", function(self, ...)
		...
		if self == SayMessageRequest  and namecallmethod == "FireServer" then
			...
			if Message:sub(1, #ankyewemkmthu) == ankyewemkmthu then return end
...
do
	RegisterCommand("crashed", function(Power)
		for i = 1,(tonumber(Power) or 22222) do
			RespawnEvent:FireServer()
		end
	end)
	...
	RegisterCommand("hshsjwhsj", function()
		Toggles["AutoRefill"] = not Toggles["AutoRefill"]
	end)
	
	RegisterCommand("battu", function()
	...
	RegisterCommand("treomay", function()
	...
	RegisterCommand("mua", function(ToolName, Ammount)
		...
		local Aliases = {
			...
			["Glock 17"] = {"glock", "sung", "gun"},
			["Revolver"] = {"revolver"},
			["Flamethrower"] = {"flamethrower"},
			["rocket launcher"] = {"rocket", "rocketlauncher"},
			...
			["booger"] = {"booger"},
			["milk"] = {"milk"},
			["orange juice"] = {"nuoccam"},
			["Bonk! Atomic punch!"] = {"bonksniperstyle","sniper","pee"}
		}
	...
	RegisterCommand("playergod", function(Target)
	...
	RegisterCommand("playertool", function(PlayerName,ToolName)
	...
	RegisterCommand("antibypass", function()
		Toggles["antibypassGuxx"] = not Toggles["antibypassGuxx"]
	...
	RegisterCommand("doiprefix", function(NewPrefix)
...
local function NewPlayerAdded(Plr)
	if table.find(Blacklist, Plr.Name) then
		StarterGui:SetCore("ChatMakeSystemMessage", {
			["Text"] = "Thang bi blacklist da vao game: "..Plr.Name,
